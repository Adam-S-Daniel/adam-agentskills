...
- **Local convergence**: setup.sh deep-merges marketplace registration
  ...
  on; their `@synced` copies off) into ~/.claude/settings.json, idempotently.
  PENDING (2026-09-24): every durable machine re-runs `bash setup.sh` in each
  home after the ADR 0013 move and uninstalls the retired marketplace's
  plugins; setup.sh does not remove the old keys.
