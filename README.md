...
| `adam-coding-local` | `/adam-coding-local:launch-wsl-claude-session` | Launch a new, top-level, interactive Claude Code session in a new Windows Terminal tab — native Windows or inside WSL — in a chosen folder, optionally remote-controllable and optionally seeded with an initial prompt or a handoff file. |
