...
**`adam-agentskills` is the single upstream registry for reusable skills.** Every other
...
- **Public (`adam-agentskills`)** — generally reusable and contains **no secrets,
...
- **Private (`adam-agentskills-private`)** — anything that embeds or depends on
...
`adam-non-coding-local` (opt-in); the private registry holds
`adam-private-anything-anywhere` — see
...
- a school bell-schedule skill — personal interest, but not sensitive. **Public is correct.** (Removed from the registry by
  [ADR 0011](docs/decisions/0011-remove-a-school-bell-schedule-skill-from-registry-and-account.md),
...
1. Pick its plugin by two questions. **Audience:** is it for coding work,
   non-coding work, or anything? **Runtime:** does it need the owner's
   machine (local files, WSL/Windows homes, a signed-in browser, local
   Cowork), or does it run anywhere, including a headless cloud session of an
   arbitrary repo? Then add it as a real directory (never a symlink) under
   that plugin's `skills/` — `plugins/adam-anything-anywhere/skills/<skill>/`,
   `plugins/adam-coding-anywhere/skills/<skill>/`,
   `plugins/adam-coding-local/skills/<skill>/` or
   `plugins/adam-non-coding-local/skills/<skill>/`. A sensitive skill goes to
...
2. Creating a **new plugin** is the rare, deliberate exception — only for a
   ...
   entry. This marketplace has no `renames` map, so a plugin name is a
   one-way door once it is enabled anywhere: renaming it means every app that
   enabled it loses it.
...
3. **Public repos publish their memory.** That is the accepted trade-off for
   ...
   default machine-local store, or moves to `adam-agentskills-private`-style hosting.
