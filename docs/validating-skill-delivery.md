...
**Expect:** skills available as `/<plugin>:<skill>` (e.g.
`/adam-coding-anywhere:skills-doctor`); the hook installs nothing.

```bash
claude plugin list                  # adam-anything-anywhere and adam-coding-anywhere should be listed
...
each."* Plugin-namespaced (`/adam-anything-anywhere:finding-unknowns`) is the pass.
...
`skills: 22/22 from … across 2 locks (adamdaniel.ai, adam-agentskills) — OK`.
