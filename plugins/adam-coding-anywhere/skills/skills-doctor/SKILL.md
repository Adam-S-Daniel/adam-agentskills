...
delivered it, `plugins/adam-coding-anywhere/skills/skills-doctor/scripts/` in a registry
...
- **The record's `registry` is a resolved git remote URL; the lock's is a
  slug.** A lock saying `Adam-S-Daniel/adam-agentskills` produces record entries
  saying `https://github.com/Adam-S-Daniel/adam-agentskills.git`, and the verdict
