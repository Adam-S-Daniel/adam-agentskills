...
class TestResolveRepos:
    def test_explicit_repos_win(self, tmp_path):
        real = tmp_path / "somewhere" / "adam-agentskills"
    ...
    def test_home_repos_layout_is_not_consulted(self, tmp_path, monkeypatch):
        ...
        directory at ~/repos/adam-agentskills outranked the checkout the script
        ...
        home = tmp_path / "home"
        decoy = home / "repos" / "adam-agentskills"
    ...
    def test_home_repos_decoy_loses_to_the_self_checkout(self, tmp_path, monkeypatch):
        ...
        decoy = home / "repos" / "adam-agentskills"
        decoy.mkdir(parents=True)
        real = tmp_path / "checkout" / "adam-agentskills"
    ...
    def test_windows_default_root_is_not_consulted(self, tmp_path, monkeypatch):
        ...
        (win_root / "adam-agentskills").mkdir(parents=True)
        monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
        monkeypatch.setattr("sync_skills._self_repo", lambda: None)

        assert win_root / "adam-agentskills" not in sync_skills.resolve_repos(None)
    ...
    def test_self_repo_is_claimed_for_its_own_name_only(self, tmp_path, monkeypatch):
        ...
        Claiming it for every declared repo is how adam-agentskills-private would
        silently resolve to the adam-agentskills clone and verify against the
        wrong tree.
        """
        monkeypatch.delenv("AGENTSKILLS_REPOS", raising=False)
        home = tmp_path / "home"
        home.mkdir()
        self_repo = tmp_path / "adam-agentskills"
        (self_repo / "plugins").mkdir(parents=True)
        monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
        monkeypatch.setattr("sync_skills._self_repo", lambda: self_repo)

        assert sync_skills._repo_candidates("adam-agentskills") == [self_repo]
        assert sync_skills._repo_candidates("adam-agentskills-private") == []
    ...
    def test_unresolvable_repo_says_it_went_unexamined(self, tmp_path, monkeypatch, capsys):
        """adam-agentskills-private can no longer be found implicitly — say so.
        ...
        err = capsys.readouterr().err
        assert "adam-agentskills-private" in err
...
class TestPluginLayout:
    ...
    def test_a_symlinked_skill_entry_is_not_the_skill(self, repo_plugin_layout):
        """A skill entry that is a symlink to another plugin's skill, sorting
        ahead of the real one ("a-personal" before "skill-a"), is not the skill.
        The uploader must find the real directory, and list each skill once."""
...
def git_clone(tmp_path):
    ...
    work = tmp_path / "adam-agentskills"
...
class TestWindowsDecoyRegression:
    ...
    because a directory at ~/repos/adam-agentskills outranked the checkout the
    ...
    def planted(self, tmp_path):
        ...
        checkout = tmp_path / "real" / "adam-agentskills"
        skill_dir = checkout / "plugins" / "adam-coding-local" / "skills" / "sync-skills"
        skill_dir.mkdir(parents=True)
        shutil.copy2(SCRIPT, skill_dir / "sync_skills.py")
        shutil.copy2(SCRIPT.parent / "account-skills.txt", skill_dir)
        real_skill = checkout / "plugins" / "adam-coding-anywhere" / "skills" / "planted-skill"
    ...
    def test_private_repo_absence_is_stated_not_guessed(self, planted):
        """A decoy at ~/repos/adam-agentskills-private must not fill the gap."""
        private_decoy = planted["home"] / "repos" / "adam-agentskills-private"
        private_decoy.mkdir(parents=True)

        proc = self._run(planted, "--dry-run", "--all")

        assert str(private_decoy) not in proc.stdout
        assert "adam-agentskills-private" in proc.stderr
...
class TestSection6JavaScriptParses:
    ...
    def test_parses_for_every_single_file_skill_in_this_repo(self, tmp_path):
        ...
        # tests/ -> sync-skills -> skills -> adam-coding-local -> plugins -> repo root
...
class TestIssue93Residuals:
    ...
    def test_expected_repo_names_warns_but_never_resolves(
        ...
        monkeypatch.setattr(
            ...
            ("adam-agentskills", "not-a-real-registry"),
        )
        monkeypatch.delenv("AGENTSKILLS_REPOS", raising=False)
...
class FakeGh:
    ...
    def __init__(self, repo="Adam-S-Daniel/adam-agentskills", issues=None,
...
class TestReportAccountUploadGap:
    ...
    REPO = "Adam-S-Daniel/adam-agentskills"
...
class TestGithubSlug:
    @pytest.mark.parametrize(
        "url",
        [
            "git@github.com:Adam-S-Daniel/adam-agentskills.git",
            "https://github.com/Adam-S-Daniel/adam-agentskills.git",
            "https://github.com/Adam-S-Daniel/adam-agentskills",
            "ssh://git@github.com/Adam-S-Daniel/adam-agentskills.git",
        ],
    )
    def test_parses_every_remote_form_this_clone_can_carry(self, url):
        assert sync_skills.github_slug_from_remote(url) == "Adam-S-Daniel/adam-agentskills"
    ...
    def test_real_slugs_still_resolve(self):
        for good in ("Adam-S-Daniel/adam-agentskills", "o/r", "a/b.c_d-e"):
...
def _tree_with(tmp_path, name, body, sub="plugins/adam-coding-local/skills"):
