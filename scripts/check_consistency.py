...
def committed_symlinks(plugins_dir: Path) -> Optional[List[str]]:
    """Every path under plugins_dir that git's INDEX records as a symlink (mode
    120000), repo-relative; None when plugins_dir is not inside a git work tree.

    The committed truth, whatever this checkout wrote to disk: on a
    core.symlinks=false clone (the Git for Windows default) a committed link is
    checked out as a small text file, which the filesystem walk in
    check_no_symlinks cannot tell from a real file. Only the index can.
    """
    if not plugins_dir.is_dir():
        return None
    inside = _git_out(plugins_dir, "rev-parse", "--is-inside-work-tree")
    if not inside or inside.strip() != b"true":
        return None
    listing = _git_out(plugins_dir, "ls-files", "-s", "-z", "--full-name", "--", ".")
    if listing is None:
        return None
    links = []
...
def check_no_symlinks(errors: List[str], plugins_dir: Path = PLUGINS_DIR) -> None:
    """No symlink anywhere under plugins/ — on disk or in the index.

    Every skill lives in exactly ONE plugin folder as a real directory (ADR
    0013). A link would be a second NAME for a skill: counted twice, zipped
    twice, installed twice, and — because git never follows one — changed by
    an edit its own plugin's diff never shows. The lock generator and the
    bootstrap hook refuse a symlinked skill directory too (ADR 0008); this is
    the same rule stated for the whole tree, so it fails here first.
    """
    if not plugins_dir.is_dir():
        return
    for dirpath, dirnames, filenames in os.walk(plugins_dir, followlinks=False):
        for name in sorted(dirnames + filenames):
            path = Path(dirpath) / name
            if path.is_symlink():
                errors.append(
                    f"{_rel(path)} is a symlink; every skill lives in exactly one "
                    "plugin folder as a real directory (ADR 0013)"
                )
    for rel in committed_symlinks(plugins_dir) or []:
        errors.append(
