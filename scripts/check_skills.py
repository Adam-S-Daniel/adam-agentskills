...
-----
  ...
  python3 scripts/check_skills.py --registry adam-agentskills=/path/to/clone --json
...
def find_skills(registry: Registry) -> List[Skill]:
    ...
    for skill_md in sorted(registry.path.glob(registry.layout)):
        ...
        # A symlinked skill directory is a second name for a skill the census
        # already reaches through its real directory; counting it would report
        # that skill as a duplicate basename of itself. This registry refuses
        # symlinks under plugins/ outright (check_consistency.py, ADR 0013); a
        # sibling registry the census reads may not.
