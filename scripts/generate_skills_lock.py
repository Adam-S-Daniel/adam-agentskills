...
GHA-bench, _agent-guidance, adam-agentskills-private, claude-memory-map,
...
DEFAULT_REGISTRY = "Adam-S-Daniel/adam-agentskills"
# The cloud-safe plugins (ADR 0013). `adam-coding-local` and
# `adam-non-coding-local` are machine-bound and opt-in, so they have no business
# in an ephemeral-surface bootstrap.
DEFAULT_BUNDLES = ("adam-anything-anywhere", "adam-coding-anywhere")
...
def digest_skill_dir(path: Path, skip: frozenset = frozenset()) -> str:
    ...
    for candidate in root.rglob("*"):
        ...
        # (adam-agentskills `plugins/`, cms-platform `skills/`, adam-agentskills-private)
...
def collect_skills(
    ...
    hook rejects WHOLESALE. `plugins/<bundle>/skills/_template/SKILL.md` generated
