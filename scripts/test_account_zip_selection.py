...
DECLARED = ["add-from-address", "finding-unknowns", "sync-skills"]
...
class TestAMalformedArtifactDegradesInsteadOfTakingTheJobOut:
    ...
    def test_a_non_string_skill_value_does_not_stop_the_valid_ones(self, tmp_path):
        ...
        out, text = run(tmp_path, report(needs_upload=["add-from-address"]),
                        latest=latest, status="reported-failure")
        assert json.loads(out["names"]) == ["add-from-address", "sync-skills"]
        assert "found drifted: sync-skills" in text
...
class TestTheSummaryStillCarriesWhatItCarried:
    ...
    def test_no_filesystem_path_of_a_skill_reaches_the_summary(self, tmp_path):
        ...
        for row in rep["skills"]:
            row["path"] = "/home/somebody/repos/adam-agentskills/" + row["name"]
