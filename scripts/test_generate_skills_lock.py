...
# The bundle every fixture registry in this file names. It is deliberately NOT
# one of this repo's real plugins: gsl.DEFAULT_BUNDLES is this repo's own
# cloud-safe pair (ADR 0013), and a fixture that silently tracked it would
# change shape every time the registry's plugin layout did.
FIXTURE_BUNDLE = "adam"

# Modes that inherit their bundles from the lock (or take none), and so must
# never be handed one by the helper below.
_INHERITING_MODES = ("--check", "--check-current", "--check-format", "--repin",
                     "--digest", "-h", "--help")


def _names_bundles_or_inherits(args) -> bool:
    return any(arg == flag or arg.startswith(flag + "=")
               for arg in args for flag in ("--bundles",) + _INHERITING_MODES)


def run_generator(*args: str, cwd: Path = None, script: Path = GENERATOR,
                  fixture_bundle: bool = True) -> subprocess.CompletedProcess:
    """Run the generator.
    ...
    A PLAIN generate that names no `--bundles` is given `--bundles
    FIXTURE_BUNDLE`, because every fixture here lays its skills out under that
    bundle and the generator's own default is this repo's real plugin pair.
    `fixture_bundle=False` turns that off — `_run_printed` does, since a
    printed remediation line must stand on its own, and the test of the
    default itself does.
    """
    args = list(args)
    if fixture_bundle and not _names_bundles_or_inherits(args):
        args += ["--bundles", FIXTURE_BUNDLE]
    return subprocess.run(
...
def _run_printed(command: str) -> subprocess.CompletedProcess:
    ...
    return run_generator(*tokens[2:], fixture_bundle=False)


def test_a_plain_generate_defaults_to_this_repos_cloud_safe_plugins(tmp_path):
    """ADR 0013: with no --bundles, the lock covers the two `-anywhere`
    plugins — the ones every ephemeral surface should get — and nothing
    machine-bound."""
    assert gsl.DEFAULT_BUNDLES == ("adam-anything-anywhere", "adam-coding-anywhere")
    root = tmp_path / "registry"
    make_registry(root, {"adam-anything-anywhere/alpha": SKILL_A,
                         "adam-coding-anywhere/beta": SKILL_B,
                         "adam-coding-local/gamma": SKILL_A})
    out = tmp_path / "skills.lock"
    proc = run_generator("--repo", str(root), "-o", str(out), fixture_bundle=False)
    assert proc.returncode == 0, proc.stderr
    lock = json.loads(out.read_text(encoding="utf-8"))
    assert lock["bundles"] == list(gsl.DEFAULT_BUNDLES)
    assert lock["registry"] == "Adam-S-Daniel/adam-agentskills"
    assert sorted(lock["skills"]) == ["adam-anything-anywhere/alpha",
                                      "adam-coding-anywhere/beta"]
...
def test_the_federated_remediation_holds_the_primary_pin_when_only_a_source_drifted(
    ...
    adam-agentskills #108 fixed this exact class for `--check-format`: `--repin`
...
_VALIDATION_CASES = [
    ...
    ("registry", "Adam-S-Daniel/adam-agentskills", True),
...
def test_the_stale_remediation_restates_the_whole_identity_it_would_otherwise_default(
    ...
    registry to `Adam-S-Daniel/adam-agentskills`, the bundles to `['adam']`, and
    lost `custom/alpha`.
...
def _emptied_source_bundle(tmp_path):
    """The registry deleted a bundle's last skill — adam-agentskills #125.
...
def test_an_empty_tree_at_a_pinned_ref_is_an_error_not_a_traceback(repin, tmp_path):
    """adam-agentskills #125: the one unusable input that escaped as a stack trace.
...
def test_a_repin_that_would_empty_a_bundle_is_refused(
    ...
    """adam-agentskills #125: the fleet bumper refusing this was never cover.
...
# =============================================================================
# ADR 0008: a symlinked skill directory is never lockable
# =============================================================================


def _account_tree(tmp_path: Path) -> Path:
    """plugins/alpha/skills/one, a real skill directory."""
    root = tmp_path / "tree"
