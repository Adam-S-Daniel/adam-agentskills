...
def test_a_fresh_machine_gets_both_marketplaces_and_the_plugins(tmp_path):
    settings = converge(tmp_path)
    assert set(settings["extraKnownMarketplaces"]) == {
        "adam-agentskills", "adam-agentskills-private"}
    assert settings["extraKnownMarketplaces"]["adam-agentskills"]["source"] == {
        "source": "github", "repo": "Adam-S-Daniel/adam-agentskills"}
    assert settings["extraKnownMarketplaces"]["adam-agentskills-private"]["source"] == {
        "source": "github", "repo": "Adam-S-Daniel/adam-agentskills-private"}
    enabled = settings["enabledPlugins"]
    assert enabled["adam-anything-anywhere@adam-agentskills"] is True
    assert enabled["adam-coding-anywhere@adam-agentskills"] is True
...
@pytest.mark.parametrize("plugin", ["adam-anything-anywhere", "adam-private-anything-anywhere"])
def test_the_account_plugins_are_off_in_terminals(tmp_path, plugin):
    """ADR 0013: the `-anything-anywhere` plugins are enabled on the claude.ai
    account too, and a terminal signed in with it would sync each as
    `<name>@synced` and load its skills a second time beside the marketplace
    install, so the durable machine turns them off by name.

    `False`, the JSON boolean — `True` would be the opposite decision and a
    missing key would leave the synced copy on."""
    settings = converge(tmp_path)
    assert settings["enabledPlugins"][f"{plugin}@synced"] is False


def test_an_operator_who_enabled_an_account_plugin_is_overridden(tmp_path):
    settings = converge(tmp_path, {"enabledPlugins": {"adam-anything-anywhere@synced": True}})
    assert settings["enabledPlugins"]["adam-anything-anywhere@synced"] is False
...
def test_the_plugin_sync_is_left_alone(tmp_path):
    ...
    syncing (Anthropic's six since 2026-09-20), and ADR 0013 turns off only
    the ones this fleet owns, `<name>@synced`, by name. Writing
...
@pytest.mark.parametrize("value", [["adam-coding-anywhere@adam-agentskills"], "on", None, 1])
...
def test_the_adr_that_decided_this_is_on_disk_and_accepted():
    ...
    assert "syncClaudeAiSkills" in adr
    index = (REPO / "docs" / "decisions" / "README.md").read_text(encoding="utf-8")
    assert "0010-let-pinned-channels-own-the-terminal.md" in index


def test_the_adr_that_named_these_plugins_is_on_disk_and_accepted():
    adr = (REPO / "docs" / "decisions"
           / "0013-start-a-fresh-public-registry-grouped-by-audience-and-runtime.md"
