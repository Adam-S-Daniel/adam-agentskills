...
A skill entry that is a symlink to another plugin's skill directory would,
on a symlink-capable checkout, be followed by setup.sh's glob
`plugins/*/skills/*/SKILL.md`, and that skill would be linked into the
per-agent homes twice under one basename. check_consistency.py refuses such a
link (ADR 0013); setup.sh skips one anyway. The section runs as
